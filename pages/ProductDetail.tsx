...
const ProductDetail: React.FC<ProductDetailProps> = ({ onAddToCart, products }) => {
  ...
  if (!product) return (
    <div className="py-48 text-center animate-fade-up px-4 flex flex-col items-center justify-center">
      ...
      <h2 className="text-3xl font-black uppercase tracking-tighter text-navy dark:text-white">Tesouro não encontrado.</h2>
      ...
      <Link to="/" className="bg-navy dark:bg-white text-white dark:text-black font-black mt-8 px-8 py-4 rounded-xl uppercase tracking-widest text-[10px] shadow-xl hover:scale-105 transition-all">
        Voltar ao Atelier
      </Link>
    </div>
  );
  ...
  return (
    <div className="pb-32 animate-fade-up px-4 md:px-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 sm:gap-20 lg:gap-32 items-start">
        ...
        <div className="flex flex-col gap-8 sm:gap-12 pt-0 sm:pt-4">
          <div className="flex flex-col gap-3 sm:gap-4">
            ...
            <h1 className="text-4xl sm:text-5xl lg:text-7xl font-black tracking-tighter text-navy dark:text-white leading-[0.9] sm:leading-[0.95] uppercase">{product.name}</h1>
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex items-baseline gap-3 sm:gap-4">
              {product?.salePrice && product.salePrice > 0 && product.salePrice < product.price ? (
                ...
                <>
                  <p className="text-4xl sm:text-5xl font-black tracking-tighter text-navy dark:text-white">{(product?.price || 0).toLocaleString()}</p>
                  <span className="text-xs sm:text-sm font-black text-primary uppercase tracking-widest mb-1 sm:mb-2">Kz</span>
                </>
              )}
            </div>
          ...
          <div className="flex flex-col gap-6 sm:gap-8">
            ...
            <div className="flex flex-col gap-3 sm:gap-4">
              <h4 className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.3em] text-navy dark:text-white">A Essência</h4>
      ...
      {/* Mobile Sticky CTA */}
      <div className="sm:hidden fixed bottom-16 left-0 right-0 z-40 p-4 bg-white/80 dark:bg-navy/80 backdrop-blur-xl border-t border-gray-100 dark:border-white/5">
        <button
          onClick={handleAddToCart}
          disabled={isAdded || product.stock === 0}
      ...
      <div className="mt-24 sm:mt-40 bg-gray-50 dark:bg-white/5 rounded-[2.5rem] sm:rounded-[4rem] p-8 sm:p-16 lg:p-24 text-center border border-gray-200 dark:border-white/5">
        ...
        <h3 className="text-4xl lg:text-5xl font-black uppercase tracking-tighter mb-12 text-navy dark:text-white">Descubra mais Tesouros</h3>
        <Link to="/" className="px-12 py-5 bg-navy dark:bg-white text-white dark:text-black rounded-2xl text-[11px] font-black uppercase tracking-widest hover:scale-105 transition-all shadow-2xl">Ver Coleção Completa</Link>
      </div>
    </div>
  );
};
