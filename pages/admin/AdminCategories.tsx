    ...
    <div className="p-6 lg:p-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 md:mb-12">
        ...
        {(userProfile?.role === 'admin' || userProfile?.permissions?.products?.create || userProfile?.permissions?.products?.edit) && (
          <button
            ...
            className="bg-primary text-black font-black px-6 md:px-8 py-3 md:py-4 rounded-xl md:rounded-2xl hover:brightness-110 transition-all flex items-center justify-center gap-2 shadow-lg shadow-primary/20 uppercase tracking-widest text-[10px] md:text-xs w-full md:w-auto"
          >
            <span className="material-symbols-outlined text-sm md:text-base">add_circle</span> Criar Categoria
          </button>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
        ...
        {(userProfile?.role === 'admin' || userProfile?.permissions?.products?.create || userProfile?.permissions?.products?.edit) && (
          <button
            onClick={() => navigate('/admin/categorias/nova')}
            className="border-2 md:border-4 border-dashed border-gray-100 dark:border-[#222115] p-6 md:p-10 rounded-2xl md:rounded-[2.5rem] flex flex-col items-center justify-center text-gray-300 hover:text-primary hover:border-primary/30 transition-all group min-h-[200px]"
          >
            <span className="material-symbols-outlined !text-4xl md:!text-6xl mb-4 group-hover:scale-110 transition-transform">add_box</span>
            <p className="font-black uppercase tracking-[0.2em] text-[10px] md:text-sm">Novo Departamento</p>
          </button>
        )}
      </div>
    </div>
