  ...
  useEffect(() => {
    const loadOverviewStats = async () => {
      ...
      try {
        ...
        const { data: allOrders, error: ordersError } = await fetchOrders(500); // Get more for stats but still capped
        const { count: productCount } = await supabase.from('products').select('*', { count: 'exact', head: true });

        if (allOrders) {
          const totalSales = allOrders.reduce((acc, order) => acc + (order.amount || 0), 0);
          const pending = allOrders.filter(o => o.status === 'PEDIDO').length;
          const paidOrders = allOrders.filter(o => o.status === 'PAGO').length;
...
return (
  <div className="p-6 md:p-10 lg:p-14 animate-fade-in max-w-[1600px] mx-auto">
