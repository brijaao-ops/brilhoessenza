    ...
    useEffect(() => {
        const checkAuth = async () => {
            try {
                ...
                if (!driverData) {
                    console.warn('Driver profile not linked to user_id:', user.id);
                    setProfileError('Conta não vinculada. Contacte a administração para associar o seu perfil de entregador.');
                    setLoading(false);
                    return;
                }
                ...
                setProfileError('Falha de autenticação. Por favor, entre novamente.');
                setLoading(false);
            }
        };
        checkAuth();
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-[#08112e] pb-24">
            ...
            {profileError && (
                <div className="min-h-screen flex flex-col items-center justify-center p-8 text-center gap-6">
                    ...
                    <div>
                        <h2 className="font-black uppercase tracking-widest text-lg mb-2">Perfil Não Encontrado</h2>
                        <p className="text-gray-400 text-sm max-w-xs mx-auto mb-6">{profileError}</p>
                        <button
            ...
            {/* Main content — only when profile loaded */}
            {!profileError && (
                <>
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                                ...
                                <div className="bg-white p-6 rounded-3xl mb-8 shadow-2xl shadow-primary/20">
                                    {selectedOrder.delivery_token ? (
                                        <QRCode
                                            value={`${window.location.href.split('#')[0]}#/checkout/confirmacao/${selectedOrder.delivery_token}`}
                                            size={200}
                                        />
                                    ) : (
