...
const MobileNav: React.FC<MobileNavProps> = ({ onOpenCart, cartCount, isAuthenticated }) => {
    ...
    return (
        <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-[60] bg-[#060e1e]/95 backdrop-blur-xl border-t border-primary/10 px-6 pb-safe pt-3 flex justify-between items-center shadow-[0_-10px_30px_rgba(0,0,0,0.3)]">
            {navItems.map((item) => {
